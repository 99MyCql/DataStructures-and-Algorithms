...
上述改进方法是动态规划的一种初级解题方式：**带备忘录的自顶向下的递归求解**。
...
从图形的角度，考虑上述示例，若采用递归算法，问题求解过程可看作一颗树，子问题是树中的节点，相同子问题会重复出现。而采用动态规划，求解过程可看作一个**有向无环图DAG**，图中节点对应**子问题（状态）**，边对应**状态间的转移**，子问题不会重复出现，且是按顺序排序的。
