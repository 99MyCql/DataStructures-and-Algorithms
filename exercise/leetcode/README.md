...
2. 遍历 `nums[i]` 和 `nums[p]` 时，要跳过重复元素。当 `nums[i+1] == nums[i]` 时，会得到一个重复结果。
...
注意：题目中，候选数组不存在重复元素，且同一个元素可以取多次。若遇到存在重复元素且一个元素只能取一次的情况，可以参考“15-三数之和”。先对数组排序，在递归/遍历过程中，本轮已被选择的元素，回溯之后的下一轮需跳过相同元素。
...
- 当 **fast && fast->next** 不为真时遍历结束。
- 结束时，若**fast不为空**，则说明链表长度为**奇数**，slow指向中间节点；若fast为空，则说明长度为偶数，slow指向后半链表的开头。
