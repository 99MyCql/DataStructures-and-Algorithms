...
我的思路：递归左右子树，回溯返回最大值最小值，判断是否满足要求。写起来太复杂，而且有很多细节需要注意，比如：区分“左子树为空”和“左子树最大值等于根节点”。

题解的方法更加巧妙：

1. 中序遍历二叉搜索树，结果一定是从小到大按序的。用 pre 记录上一个遍历的值，当前节点一定满足：`root->val > pre` 。
2. 递归子树时，传入区间 `[min,max]`，子树中的值一定在区间内。且左子树 `[min,root->val]` ，右子树 `[root->val,max]` 。

注意细节：由于节点值有可能等于INT_MAX，因此最大值应更大，取LLONG_MAX。
...
注意细节：当遍历完一个链表时，必须要让 pA 和 pB 指向 null ，不然遇到不相交的链表会死循环。
...
## 233 - 💡数字 1 的个数 - 困难 - 数位DP

参考了灵神题解：

1. 通过一个 cnt 状态记录当前数字中 1 的个数，理解之后，思路更清晰，代码更简洁。
2. 可以不需要前导零。
...
参考题解：<https://leetcode.cn/problems/implement-rand10-using-rand7/solutions/167850/cong-zui-ji-chu-de-jiang-qi-ru-he-zuo-dao-jun-yun-/>
...
数位DP模板，见：[note\algorithms\dp.md](../../note/algorithms/dp.md)
...
## 3006 - 找出数组中的美丽下标 I - 中等 - 二分

## 3007 - 💀价值和小于等于 K 的最大数字 - 中等 - 数位DP+二分

太难了！

解题关键：

1. num 的价值与 num 大小成正比，可用二分找最佳答案。
2. 将数字转为二进制字符串进行数位DP时，二进制字符串一定是从高位到低位，因为数位DP中低位受高位约束。

## 3008 - ⭐找出数组中的美丽下标 II - 困难 - KMP+二分

注意：STL string库的find函数的时间复杂度为 O(m*n) ！！！
