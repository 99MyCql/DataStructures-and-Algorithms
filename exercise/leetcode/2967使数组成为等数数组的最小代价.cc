...
public:
    ...
    long long minimumCost(vector<int>& nums) {
        ...
        vector<int> hws;
        if (n%2 == 1) {
            hws.push_back(findL(nums[n/2]));
            hws.push_back(findR(nums[n/2]));
        } else {
            hws.push_back(findL(nums[n/2-1]));
            hws.push_back(findR(nums[n/2-1]));
            hws.push_back(findL(nums[n/2]));
            hws.push_back(findR(nums[n/2]));
        }
...
vector<int> initPal() {
    ...
    pals.push_back(1000000001);
    return pals;
}

vector<int> pals = initPal();

class Solution {
public:
    long long cost(vector<int>& nums, int pal) {
        long long c = 0;
        for (auto num : nums) {
            c += abs(num - pal);
        }
    ...
    long long minimumCost(vector<int>& nums) {
        sort(nums.begin(), nums.end());
        int n = nums.size();
        int p = lower_bound(pals.begin(), pals.end(), nums[(n-1)/2]) - pals.begin();
        return min(cost(nums,pals[p-1]), cost(nums,pals[p]));
    }
};