...
public:
    ...
    void init() {
        memset(nodes, 0, sizeof(Node)*MAX);
    }

    void createNode(int fa, int son) {
        printf("fa:%d, son:%d\n", fa, son);
        if (nodes[fa].son == 0) nodes[fa].son = son;
        else {
            int p = nodes[fa].son;
            while (nodes[p].bro) p = nodes[p].bro;
            nodes[p].bro = son;
        }
        nodes[son].fa = fa;
    }

    void send2son(int root, int coins) {
        nodes[root].coins += coins;
        addFaCoins(root, coins);
        if (nodes[root].son) send2son(nodes[root].son, coins);
        if (nodes[root].bro) send2son(nodes[root].bro, coins);
    }

    void addFaCoins(int root, int coins) {
        int p = nodes[root].fa;
        while (p) {
            nodes[p].son_coins_sum += coins;
            p = nodes[p].fa;
        }
    }
    ...
    vector<int> bonus(int n, vector<vector<int>>& leadership, vector<vector<int>>& operations) {
        ...
        for (int i=0; i < len; i++) {
            createNode(leadership[i][0], leadership[i][1]);
        }
        ...
        for (int i=0; i < len; i++) {
            ...
            switch (operations[i][0]) {
                // 给团队的一个成员（也可以是负责人）发一定数量的LeetCoin；
                case 1:
                    nodes[operations[i][1]].coins += operations[i][2];
                    addFaCoins(operations[i][1], operations[i][2]);
                    break;
                // 给团队的一个成员（也可以是负责人），以及他/她管理的所有人（即他/她的下属、他/她下属的下属，……），发一定数量的LeetCoin；
                case 2:
                    nodes[operations[i][1]].coins += operations[i][2];
                    addFaCoins(operations[i][1], operations[i][2]);
                    if (nodes[operations[i][1]].son) send2son(nodes[operations[i][1]].son, operations[i][2]);
                    break;
                // 查询某一个成员（也可以是负责人），以及他/她管理的所有人被发到的LeetCoin之和。
                case 3:
                    ans.push_back((nodes[operations[i][1]].son_coins_sum + nodes[operations[i][1]].coins) % MOD);
                    break;
                default:
                    break;
            }
